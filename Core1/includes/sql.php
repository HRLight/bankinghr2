           ...
           function  collection($table, $name){
            ...
            $sql = $db->query("SELECT p.*, s.Name FROM $table p LEFT JOIN status s ON s.Status_Code = p.Co_Status WHERE p.Co_Code='$name';");
              if($result = $db->fetch_assoc($sql))
                ...
                /* select notification for Budget
                /*--------------------------------------------------------------*/
              function  Call_Pending_Invoices(){
                  global $db;
                  $sql = "SELECT * FROM loans WHERE Co_Status='102';" or die($mysqli->error);
                  return find_by_sql($sql);
                }
                /*--------------------------------------------------------------*/
                /* select notification for Budget
                /*--------------------------------------------------------------*/
              function  Call_deposit_Invoices(){
                  global $db;
                  $sql = "SELECT * FROM deposit WHERE Co_Status='102';" or die($mysqli->error);
                  return find_by_sql($sql);
                }
                /*--------------------------------------------------------------*/
                /*  Function for select * loans
                /*--------------------------------------------------------------*/
                function Find_loan($table,$id)
                {
                  global $db;
                  $id = (int)$id;
                    if(tableExists($table)){
                          $sql = $db->query("SELECT * FROM {$db->escape($table)} WHERE A_Number='{$db->escape($id)}' LIMIT 1");
                          if($result = $db->fetch_assoc($sql))
                            return $result;
                          else
                            return null;
