						...
						<div style="max-height:600px">
							<table id="example" class="table table-bordered data-table" style="width:100%">
								...
								<?php
									...
									$name = $user['name'];
									$user_level = $user['user_level'];
									$conn = new mysqli('localhost', 'u811015322_Systembanking6', 'Systembanking6@gmail', 'u811015322_obms_db');
									
									if ($user_level <= 1){
										$query = $conn->query("SELECT * FROM time_and_attendance ORDER BY time_id DESC");
									}
									else if ($user_level = 2){
										$query = $conn->query("SELECT * FROM time_and_attendance WHERE user_level >= 2 ORDER BY time_id DESC");
									}
									else {
										$query = $conn->query("SELECT * FROM time_and_attendance WHERE username = '$username' ORDER BY time_id DESC");
									}
