...
<?php
	...
	$working = 0;
	$query1 = $conn->query("SELECT working FROM `time_and_attendance` WHERE `user_id` = '$username'") or die(mysqli_error());
	...
	<div class="col-md-12">
		<div class="panel">
			<div class="jumbotron text-center">
				<?php
					$query = $conn->query("SELECT complaint_notif FROM users WHERE employee_id='$user_id'");
				...
				<?php 
					...
					$q_student = $conn->query("SELECT * FROM `tblschedule` WHERE empid = $user_id AND status = 1") or die(mysqli_error($q_student));
			...
			<?php			
				if(isset($_POST['login'])) {
					...
					$q_student = $conn->query("SELECT * FROM `users` WHERE `employee_id` = '$username'") or die(mysqli_error());
					$f_student = $q_student->fetch_array();
					$conn->query("INSERT INTO `time_and_attendance` (user_id,login_time,username,name,user_level,working)
					VALUES('$username', '$login_time', '$username', '$name', '$user_level', '1')") or die(mysqli_error());
					header("Location:time_index_loggedin.php");  
				}
				...
				if(isset($_POST['logout'])) {
					...
					$query = $conn->query("SELECT MAX(time_id) FROM time_and_attendance");
					$row = $query->fetch_array();
					$last = $row[0];
					
					$q_student = $conn->query("SELECT * FROM `users` WHERE `username` = '$username'") or die(mysqli_error());
					$f_student = $q_student->fetch_array();
					$conn->query("UPDATE time_and_attendance SET logout_time='$logout_time' WHERE time_id='$last'") or die(mysqli_error());
					
					$get_login_time = $conn->query("SELECT login_time FROM time_and_attendance WHERE time_id='$last'");
					...
					$conn->query("UPDATE time_and_attendance SET calculated_work='$result' WHERE time_id='$last'");
					$conn->query("UPDATE time_and_attendance SET working='0' WHERE time_id='$last'");
				}
			?>
