...
/*--------------------------------------------------------------*/
  function authenticate($username='', $password='') {
   ...
   function find_all_leave() {
    global $db;
	$sql = "SELECT leave_request.*, employees.* FROM leave_request JOIN employees ON leave_request.employee_id = employees.employee_id";
	$resi = find_by_sql($sql);
...
if($row>0){
  $sql = "SELECT employees.*,positions.*,departments.* FROM employees JOIN positions ON employees.pos_id = positions.pos_id JOIN departments ON employees.dept_id = departments.dept_id WHERE employees.employee_id = '$id'";
  $result = $db->query($sql);
  $info = $result->fetch_assoc();

  $json = array('name' => $info['first_name']." ".$info['last_name'],'average' => $average,'date' => $date,'id' => $info['employee_id'],'pos' => $info['pos_name'], 'dept' => $info['dept_name']);
}
return $json;
}

function get_resigned($month){
    global $db;
    $sql = "SELECT COUNT(*) as count FROM resigned_employees WHERE MONTH(date_resigned) = MONTH('$month')";
    $res = $db->query($sql);
    $res_json = $res->fetch_assoc();
    $num = $res_json['count'];
    
    return $num;
    
}
function get_hired($month){
    global $db;
    $sql = "SELECT COUNT(*) as count FROM employees WHERE MONTH(date_hired) = MONTH('$month')";
    $res = $db->query($sql);
    $res_json = $res->fetch_assoc();
    $num = $res_json['count'];
...
function total_manager(){
    global $db;
    $sql = "SELECT COUNT(*) as count FROM employees WHERE pos_id = 6 OR pos_id = 7 OR pos_id = 11";
    $sql .= " OR pos_id = 20 OR pos_id = 24 OR pos_id = 22 OR pos_id = 27 OR pos_id = 32 OR pos_id = 36";
     $res = $db->query($sql);
    $res_json = $res->fetch_assoc();
    $num = $res_json['count'];
...
function total_budget(){
    global $db;
    $sql = "SELECT * FROM obudget WHERE id = 1";
     $res = $db->query($sql);
    $res_json = $res->fetch_assoc();
    $num = $res_json['HR4'];
  ...
  function total_leave(){
      ...
      $q = "SELECT COUNT(*) as lev FROM leave_request WHERE status = 'approved'";
      $res = $db->query($q);
