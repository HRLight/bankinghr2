<?php
	...
	if(ISSET($_REQUEST['id'])){
		...
		$query=mysqli_query($conn, "SELECT * FROM `archive_complain` WHERE `id`='$id'") or die(mysqli_error());
		...
		mysqli_query($conn, "DELETE FROM `archive_complain` WHERE `id`='$id'") or die(mysqli_error());
		
		echo"<script>alert('Data successfully transfer')</script>";
		echo"<script>window.location='archive_complain.php'</script>";
	}