...
<?php
  if(isset($_POST['applicationform'])){
   ...
   $imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
     $folder = '../Logistic2/contract/'.basename($_FILES["fileToUpload"]["name"]);
   ...
   if(empty($errors)){
       ...
       $body   = remove_junk($db->escape($_POST['body']));
        $query = "INSERT INTO logistic2files (";
        ...
        if($db->query($query)){
          ...
          $queryy .=") VALUES (";
          $queryy .="'{$users_id}','Sent Supplier Contract to Logistic2 $name', '{$folder}', 'ADMINISTRATIVE', '{$date}'";
          $queryy .=")";
          if($db->query($queryy)){
    //ending
...
<div class="panel-body">
      ...
      <head>
        ...
        <div id="ApplicationForm" class="tabcontent">
            <form method="post" action="logistic2contract.php" autocomplete="off" enctype="multipart/form-data"> 
