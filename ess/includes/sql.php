...
/*--------------------------------------------------------------*/
 ...
 /*--------------------------------------------------------------*/
   function authenticate_v2($username='', $password='') {
     ...
     $username = $db->escape($username);
     $password = $db->escape($password);
     $sql  = sprintf("SELECT * FROM ess_users WHERE employee_id ='%s' LIMIT 1", $username);
     $result = $db->query($sql);
     if($db->num_rows($result)){
       $user = $db->fetch_assoc($result);
       $password_request = sha1($password);
  ...
  function current_user(){
      ...
      global $db;
      if(!$current_user){
         if(isset($_SESSION['ess_id'])):
             $user_id = intval($_SESSION['ess_id']);
 ...
 function check_working(){
      ...
      $id = $_SESSION['ess_id'];
      $date = date('Y-m-d');
      $sql = "SELECT working FROM time_attendance WHERE date = '$date' AND employee_id = '$id'";
      $res = $db->query($sql);
      $working = $res->num_rows;
      if($working >0){
  ...
  function find_user(){
      global $db;
      $id = intval($_SESSION['ess_id']);
      
      $sql = "SELECT employees.*,positions.*,departments.*,ess_users.* FROM employees JOIN positions ON employees.pos_id";
      $sql .= " = positions.pos_id JOIN departments ON employees.dept_id = departments.dept_id JOIN ess_users ON ";
      $sql .= " employees.employee_id = ess_users.employee_id WHERE ";
      $sql .= "employees.employee_id = '$id' LIMIT 1";
      $result = $db->query($sql);
 ...
 function updateLastLogIn($user_id)
	{
		global $db;
    $date = make_date();
    $sql = "UPDATE ess_users SET last_login='{$date}' WHERE id ='{$user_id}' LIMIT 1";
    $result = $db->query($sql);
