                ...
                <tbody>
					...
					<tr>
						...
						<td class="p-0">
							...
							<?php
							$jitems = $db->query("SELECT j.*,a.name as account, g.type as `type` FROM `journal_items` j inner join account_list a on j.account_id = a.Co_Code inner join group_list g on j.group_id = g.id where j.journal_id = '{$row['id']}' and a.definedStat = '104' ORDER by type");
