        ...
        <div class="container-fluid" id="outprint">
			...
			<table class="table table-hover table-bordered">
                ...
                <tbody>
                    ...
                    $accounts = $db->query("SELECT * FROM `account_list`  where Co_Code in (SELECT account_id FROM `journal_items` where journal_id in (SELECT id FROM `general_journal` where date(journal_date) BETWEEN '{$from}' and '{$to}' ) AND definedStat = '104')");
                    while($arow = $accounts->fetch_assoc()):
                        $items = $db->query("SELECT j.*,g.type FROM `journal_items` j inner join group_list g on j.group_id = g.id where j.account_id = '{$arow['Co_Code']}' and j.journal_id in (SELECT id FROM `general_journal` where date(journal_date) BETWEEN '{$from}' and '{$to}');");
                    ?>
                    <tr>
...
<script>
	$(document).ready(function(){
            $('#filter').submit(function(e){
                e.preventDefault()
                location.href="working_trial_balance.php?page=reports/trial_balance&"+$(this).serialize();
            })
