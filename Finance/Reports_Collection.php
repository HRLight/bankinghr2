...
<div class="card card-outline card-primary">
	...
	<div class="card-body">
        ...
        <div class="container-fluid" id="outprint">
            ...
            <h5 class="text-center"><b>Collection Reports</b></h5>
            <?php if($from == $to): ?>
			...
			<table class="table table-hover table-bordered">
                ...
                <tbody>
                    ...
                    $prefix = date("Ym-");
                    $balance = 0;
                    $col_journal = $db->query("SELECT * FROM `budgetreleasing` WHERE DATE(P_Date) BETWEEN '{$from}' AND '{$to}' AND P_Status = '105';");
                    while($row = $col_journal->fetch_assoc()):
                    ?>
                        <tr>
                            <td><?= $row['P_Date'];  ?></td>
                            <td><?= $row['P_Purpose'];  ?></td>
                            <td><?= $journalCode = "".$prefix."".$row['P_Code'];  ?></td>
                            <td class="text-right"><?= $row['P_Amount']; ?></td>
                            <?php $balance += $row['P_Amount']; ?>
                        </tr>
                      <?php endwhile; ?>
                </tbody>
                <tfoot>
