...
// ======================================================================================================================================================================================================================== -->
  if ($Table==="reimbursment") { // checking if the table data required is reimbursment...
    $sql="SELECT p.*, s.Name FROM $Table p LEFT JOIN status s ON s.Status_Code = p.Co_Status Where  p.Co_Code='$userid'";
    $result = $db->query($sql);
    if ($result->num_rows > 0) {
    while( $row = mysqli_fetch_array($result) ){
  $Code=$row['Co_Code'];
  ...
  $response .= " <div class='row'>";
      ...
      $response .= "<div>";
        $response .= "<ul type='none'>";
          ...
          $response .= "<li style='flex-basis: 40.5%;'>".number_format($Total, 2, '.', ',')."</li>";
          $response .= "</div>";
          $response .= "<div style='display: flex;''>";
