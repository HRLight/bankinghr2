<?php
  ...
  $query = "SELECT job_vacancy.*, positions.*, departments.* FROM job_vacancy JOIN positions ON job_vacancy.pos_id = ";
  $query .= "positions.pos_id JOIN departments ON positions.dept_id = departments.dept_id WHERE positions.pos_name = '$position'";
  $res = $db->query($query);
  $depa = $res->fetch_assoc();
  $dept = $depa['dept_id'];
  $real_position = $depa['pos_id'];

 
  $sql ="INSERT INTO employees (first_name,middle_name,last_name,email,contact_number,birth_date,age,gender,civil_status,religion,address,pos_id,date_hired,dept_id)";
  $sql .= " VALUES ('$fname','$mname','$lname','$email','$contact','$bday','$age','$gender','$civil_status','$religion','$address','$real_position','$today','$dept')";
  if($db->query($sql)){
