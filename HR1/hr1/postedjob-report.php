...
$sql = "SELECT posted_jobs.*,job_vacancy.*,positions.*,job_qualifications.* FROM posted_jobs JOIN job_vacancy ON posted_jobs.job_id = job_vacancy.job_id JOIN positions ON job_vacancy.pos_id = positions.pos_id JOIN job_qualifications ON positions.pos_id = job_qualifications.pos_id WHERE posted_jobs.status = 'posted'";
$result = $db->query($sql);
