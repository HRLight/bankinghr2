...
$sql = "SELECT employees.*,appreciation_awards.*,positions.*,departments.* FROM employees JOIN appreciation_awards ON ";
$sql .="employees.employee_id = appreciation_awards.employee_id JOIN positions ON employees.pos_id = positions.pos_id JOIN departments ON employees.dept_id = departments.dept_id WHERE appreciation_awards.status = 'pending'";
$result = $db->query($sql);
...
if($stat['status'] == 'printed'){
          ...
          <?php } ?>
            ...
            <?php }else { ?>
                <a href="request-app-certificate.php?id=<?php echo $info['employee_id'];?>&tit=<?php echo $info['award_title']?>" class=" btn btn-sm btn-success"  title="">
