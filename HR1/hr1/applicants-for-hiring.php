          ...
          <tr>
           ...
           if($info['status'] == "hired"){ 
            ...
            echo '<td class="text-center"><a href="remove-result.php?id='.$info['applicant_id'].'" class="btn btn-danger btn-sm">remove</a>';
             ...
             <div class="btn-group">
             <?php
             $emp_id = $info['applicant_id'];
              $sql = "SELECT * FROM employee_documents WHERE applicant_id = '$emp_id'";
$res = $db->query($sql);
$num = $res->num_rows;
if($num>0){
?>
                     <a href="view_documents.php?id=<?php echo $info['applicant_id']; ?>" class="btn btn-sm btn-success">View Document</a>
               <?php }else{ echo "uploading documents"; }?>
                </div>
               <?php } } ?>
             </td>
          </tr>
