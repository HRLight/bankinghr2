...
$sql = "INSERT INTO newhire_training (employee_id,date_start,date_end,department) VALUES ('$id','$date_start','$date_end','$dept');";
if($db->query($sql)){
...
<?php 
				$sql ="SELECT employees.*, departments.* FROM employees JOIN departments ON employees.dept_id = departments.dept_id WHERE employee_id = '$id';";
				$result = $db->query($sql);
				$dept = $result->fetch_assoc();
					
				?>
			<input name="dep" type="text" disabled style="width: 300px; height: 35px;" value="<?php echo $dept['dept_name'];?>">
