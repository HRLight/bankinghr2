...
$sql = "SELECT contract_signing.*, employees.*,new_hires.*,positions.* FROM contract_signing JOIN employees ON contract_signing.employee_id = employees.employee_id";
$sql .= " JOIN new_hires ON contract_signing.employee_id = new_hires.employee_id JOIN positions ON employees.pos_id = positions.pos_id  WHERE new_hires.status = 'ongoing contract signing' OR new_hires.status = 'for orientation'";
$result = $db->query($sql);
