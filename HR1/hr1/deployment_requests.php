...
$sql = "SELECT new_hires.*,employees.*, positions.* FROM new_hires JOIN employees ON new_hires.employee_id = employees.employee_id";
$sql .= " JOIN positions ON employees.pos_id = positions.pos_id WHERE new_hires.status = 'requesting for deployment approval'";
$result = $db->query($sql);
