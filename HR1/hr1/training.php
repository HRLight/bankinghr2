...
$sql =" SELECT employees.*,newhire_training.*,new_hires.*,positions.* FROM employees JOIN newhire_training ON employees.employee_id = newhire_training.employee_id ";
$sql .= "JOIN new_hires ON employees.employee_id = new_hires.employee_id JOIN positions ON employees.pos_id = positions.pos_id";
$result = $db->query($sql);
 ...
 <?php if($row>0){ ?>
        ...
        <?php do{ ?>
          <tr>
           ...
           <?php }else{  ?> <a href="#" class="btn btn-secondary btn-sm">View</a>  <?php } }else{   ?>
