...
$sql = "SELECT employees.*,training_management.*,positions.*,departments.* FROM employees JOIN training_management ON ";
$sql .="employees.employee_id = training_management.employee_id JOIN positions ON employees.pos_id = positions.pos_id JOIN departments ON employees.dept_id = departments.dept_id WHERE training_management.status = 'complete'";
$result = $db->query($sql);
