...
/*--------------------------------------------------------------*/
 ...
 function find_all_proc(){
      global $db;
      $results = array();
      $sql ="SELECT * FROM request_procurement WHERE department='HR1' ORDER BY id ASC";
...
function find_all_password_request() {
   global $db;
   $sql = "SELECT change_pass_request.*,employees.*,positions.*,departments.* FROM change_pass_request JOIN employees ON ";
   $sql .= "change_pass_request.employee_id = employees.employee_id JOIN positions ON employees.pos_id = positions.pos_id JOIN departments ON employees.dept_id = departments.dept_id WHERE change_pass_request.department = 'HR1' AND change_pass_request.status = 'pending'";
   $res = find_by_sql($sql);
 ...
 /*--------------------------------------------------------------*/
  ...
  function find_all_user(){
   ...
   function find_all_user_hr(){
      ...
      $sql .="FROM users u ";
      $sql .="LEFT JOIN user_groups g ";
      $sql .="ON g.group_level=u.user_level WHERE department = 'HR1' ORDER BY u.name ASC";
      $result = find_by_sql($sql);
      return $result;
  }
  /*--------------------------------------------------------------*/
