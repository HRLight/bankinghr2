<?php
  ...
  $sql = "SELECT * FROM training_sched WHERE t_id = '$id'";
  $res = $db->query($sql);
  $ext = $res->fetch_assoc();
  ...
  if($ext['external'] == 'no'){
   ...
   if($db->query($q)){
       ...
       $sql = "UPDATE training_sched SET status ='pending' WHERE t_id = '$id'";
        if($db->query($sql)){
    ...
    $sql = "UPDATE training_sched SET status ='approved' WHERE t_id = '$id'";
        if($db->query($sql)){
       $_SESSION['status'] =  "Traing Schedule Approved";
    $_SESSION['status_code'] =  "success";
    redirect('training_approval.php',false);
