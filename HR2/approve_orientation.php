<?php
  ...
  $sql = "SELECT * FROM orientation WHERE employee_id = '$id'";
  $res = $db->query($sql);
  $ext = $res->fetch_assoc();
  ...
  if($ext['external'] == 'no'){
   ...
   if($db->query($q)){
       ...
       $sql = "UPDATE orientation SET status ='pending' WHERE employee_id = '$id'";
        if($db->query($sql)){
       $_SESSION['status'] =  "Orientaion Schedule Approved";
    ...
    $sql = "UPDATE orientation SET status ='approved' WHERE employee_id = '$id'";
        if($db->query($sql)){
       $_SESSION['status'] =  "Orientation Approved";
    $_SESSION['status_code'] =  "success";
    redirect('training_approval.php',false);
}
}